...
function CallPrint2Time(strid1,strid2)
{
...
//Tao lai cac button voi Nescast4 va mozilla5
...
//------------------------------------------------------------------------------------------------------------
// Panel functions for Netscape 4(Ham dung cho NetsCape4) Ns4
//------------------------------------------------------------------------------------------------------------
...
function b_mOver_ns4(nr) 
{
	...
	l.write("' border=0></A></SPAN><Font size=2 face=Times New Roman color=Red>");
...
//------------------------------------------------------------------------------------------------------------
// Panel functions for ie4, ie5, ns5, op5(Ham dung cho IE4,IE5,Netscep5,Firefox)
//------------------------------------------------------------------------------------------------------------
...
function createPanel(name,caption) 
{
	...
	if (!bt.ns4) 
	{                     // functions for all browsers but ns4
...
//-------------------------------------------------------------------------------------------------------------
//Chinh herder cua tieu de o day----------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------
...
//	document.write('.button {width:300;text-align:Center; font-family:Times New Roman;');
//	document.write('font-size:13pt; cursor:hand; border-width:2;');
//
//	document.write('-webkit-border-radius: 10px;');
//	document.write('-moz-border-radius: 10px;');
//	document.write('border-radius: 10px;');
//	document.write('-webkit-box-shadow: #FACC3E 1px 1px 1px;');
//	document.write('-moz-box-shadow: #FACC3E 1px 1px 1px;');
//	document.write('box-shadow: #FACC3E 1px 1px 1px;');
//	document.write('background: green;');
//	document.write('opacity:0.8;');
//	document.write('background: -webkit-gradient(linear, 0 0, 0 bottom, from(#FACC3E), to(#350000));');
//	document.write('background: linear-gradient(#FACC3E, ##350000);');
//	document.write('-pie-background: linear-gradient(#FACC3E, #350000);');
//	
...
//
//	//sonpn
	document.write('.button :hover{opacity:1!important;}');

	
//Chinh ten ben trong menu
//-------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------
	//document.write('.imgB {color:black; font-family:Times New Roman; font-size:12pt;background:white!important;}');
...
function b_draw() 
{
	...
	var j;
	var t=0;
	var h; 					//Chieu cao trong khung
	var c=0;
	this.writeStyle();
	if (bt.ns5 || bt.op5) c=6;       //two times border width
	if (bt.ns4) 
	{                 //draw OutlookBar for ns4
		//OutlookBar layer..
		...
		for (i=0;i<this.panels.length;i++) 
		{
			...
			document.write(' top='+i*30+' bgcolor='+this.bgcolor);
			document.write(' clip="0,0,'+this.width+',');
			document.write(this.height-(this.panels.length-1)*30+'">');
		   //one layer to host the panel buttons
			...
			document.write(this.width+',30" bgcolor=silver class=button ');
			...
			document.write('<Font  class=noLine>'+this.panels[i].caption);
			...
			document.write('width=22 height=22 src=up32.png border=0>');
			document.write('</A></LAYER><Layer top=');
			document.write((this.height-(this.panels.length)*30)+'<Layer top=');
			document.write((this.height-(this.panels.length)*30)+' left=');
			document.write((this.width-20)+'><A href="#" onClick="');
			document.write(this.panels[i].v+'.up(16);" onmouseOver="');
			document.write(this.panels[i].v+'.clear();"><img width=22 height=22 ');
			document.write('src=down32.png border=0></A></LAYER>');
			document.write('</LAYER>');
		}
		document.write('</LAYER>');
	}
	else 
	{                             //draw Outlook bar for all browsers but ns4
		//OutlookBar layer..
		document.write('<DIV class="sonpn-1" id='+this.name+' Style="position:absolute; left:');
		document.write(this.xpos+'; top:'+this.ypos+'; width:'+this.width);
		document.write('; height:'+this.height+'; background-color:'+this.bgcolor)
		document.write('; clip:rect(0,'+this.width+','+this.height+',0)">');
		h=this.height-((this.panels.length-1)*30) 
		//one layer for every panel...
		for (i=0;i<this.panels.length;i++) 
		{
			document.write('<DIV class="sonpn-2" id='+this.name+'_panel'+i);
			...
			//one layer to host the panel buttons
			document.write('<div class="sonpn-3" id='+this.name+'_panel'+i);
			...
			for (j=0;j<this.panels[i].img.length;j++) 
			{
				if (bt.op5) 
				{
					...
					document.write('; top:'+mtop+'; text-align:center;">');
					document.write('<a class="sonpn-8"><img src='+this.panels[i].img[j]+' class=imgnob ');
					document.write('onmouseOver="'+this.panels[i].v+'.mOver('+j);
					document.write(');"><BR>'+this.panels[i].lbl[j]+'</a></DIV>');
				}
				else 
				{
			...
			if (!bt.op5) 
			{
				...
				document.write((this.width-c)+'; height:'+(30-c)+';"><A href="#" ');
				document.write('onClick="'+this.v+'.showPanel('+i+');this.blur();');
				document.write('return false;" class=noLine><FONT  ');
			...
			document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
			document.write('<img width=22 height=22 src=up32.png border=0>');
			...
			document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
			document.write('<img width=22 height=22 src=down32.png border=0>');
			document.write('</A></DIV>');
...
//Outlook Bars zu verdecken!
		if (bt.op5) 
		{
			...
			for (i=0;i<this.panels.length;i++) 
			{
				...
				document.write(');" style="position:absolute; left:0; top:0; width:');
				document.write((this.width-c)+'; height:'+(30-c)+';">');
...
function b_showPanel_ns4(nr) 
{
	...
	for (i=0;i<l;i++) 
	{
		if (i>nr) 
		{
			this.panels[i].obj.top=this.height-((l-i)*30)-1;
		}
		else 
		{
			this.panels[i].obj.top=i*30;
		}
	}
}
...
function b_showPanel(nr) 
{
	...
	for (i=0;i<l;i++) 
	{
		if (i>nr) 
		{
			this.panels[i].obj.style.top=this.height-((l-i)*30);
			...
			if (bt.op5) 
			{
				...
				this.panels[i].objc.style.top=this.height-((l-i)*30);
			}
		}
		else 
		{
			this.panels[i].obj.style.top=i*30;
			//Opera doesn't support clip:rect()!
			...
			if (bt.op5) 
			{
				...
				this.panels[i].objc.style.top=i*30;
			}
		}
	}
}
	...
	else 
	{
		...
		else 
		{
			...
			for (i=0; i<this.panels.length; i++) 
			{
				...
				this.panels[i].objm2.style.top=(this.height-(this.panels.length)*30);
...
//--------------------------------------------------------------------------------------------------------
//b_Draw la ham quan trong can chinh div giup ve menu
//--------------------------------------------------------------------------------------------------------
...
function startclock()
{
	...
	if (nmins<10)
	 nmins="0"+nmins;
	document.getElementById('mylabel').innerHTML="<font color=#ffffff size=10>"+nhours+":"+nmins+":"+nsecn+" "+AorP+"</font>";
